...
from mmengine.model.base_model.data_preprocessor import BaseDataPreprocessor, ImgDataPreprocessor
from mmdet.models.data_preprocessors.data_preprocessor import DetDataPreprocessor
from typing import List, Mapping, Tuple, Optional, Sequence, Union
import random
...
class HD_BSNet_ImgDataPreprocessor(ImgDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> Union[dict, list]:
        ...
        #for restriction mse loss
        imgs_src = copy.deepcopy(_batch_inputs) 
        ...
        if is_seq_of(_batch_inputs, torch.Tensor):
            ...
            for img_src in imgs_src:
                ...
                # Convert to float after channel conversion to ensure
                # efficiency
                img_src = img_src.float()

                if self._enable_normalize:
                    img_src = img_src / 255
                img_inputs.append(img_src)

            img_inputs = stack_batch(img_inputs, self.pad_size_divisor,
                                       self.pad_value)
        # Process data with `default_collate`.
            ...
            if self._channel_conversion:
                ...
                imgs_src = imgs_src[:, [2, 1, 0], ...]
            # Convert to float after channel conversion to ensure
            # efficiency
            _batch_inputs = _batch_inputs.float()
            imgs_src = imgs_src.float()

            if self._enable_normalize:
                _batch_inputs = (_batch_inputs - self.mean) / self.std
                imgs_src = imgs_src / 255
            ...
            img_inputs = F.pad(imgs_src, (0, pad_w, 0, pad_h),
                                 'constant', self.pad_value)
        else:
        ...
        data['src_inputs'] = img_inputs
        return data
        

@MODELS.register_module()
class HD_BSNet_DetDataPreprocessor(HD_BSNet_ImgDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        inputs, data_samples, src_inputs = data['inputs'], data['data_samples'], data['src_inputs']
        ...
        return {'inputs': inputs, 'data_samples': data_samples, 'src_inputs': src_inputs}
